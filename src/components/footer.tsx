              ...
              <FooterList title="Company" items={navigation.company} />
                ...
                <div className='mt-16 flex justify-center space-x-10'>
                    {navigation.social.map((item) => (
                        <Link key={item.name} href={item.href} target={item.target} className='text-gray-400 hover:text-gray-500'>
...
function FooterList(props: { title: string; items: { name: string; href: string; target?: string }[] }) {
    return (
        <>
            <h3 className='text-sm font-semibold leading-6 text-foreground'>{props.title}</h3>
            <ul role='list' className='mt-6 space-y-4'>
                {props.items.map((item) => (
                    <li key={item.name}>
                        <a href={item.href} target={item.target} className='text-sm leading-6 text-foreground/60 hover:text-foreground'>
                            {item.name}
                        </a>
                    </li>
                ))}
